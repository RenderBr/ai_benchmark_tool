...
def get_models() -> List[EchoModel]:
    ...
    models = []
    for conf in configs:
        cls = MODEL_CLASSES.get(conf.type)
        if cls:
            models.append(cls())
    return models
...
@app.get("/admin/models", response_model=List[ModelConfig])
def list_models(user: User = Depends(require_admin)):
    with get_session() as session:
        models = session.exec(select(ModelConfig)).all()
        return models


@app.post("/admin/models", response_model=ModelConfig)
def add_model(data: ModelCreate, user: User = Depends(require_admin)):
    if data.type not in MODEL_CLASSES:
        raise HTTPException(status_code=400, detail="Unknown model type")
    with get_session() as session:
        m = ModelConfig(name=data.name, type=data.type)
        session.add(m)
        session.commit()
        session.refresh(m)
        return m
...
def delete_model(model_id: int, user: User = Depends(require_admin)):
    with get_session() as session:
